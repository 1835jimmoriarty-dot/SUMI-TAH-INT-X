...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/metrics')
      .then((r) => r.json())
      .then((d) => {
        if (d && d.telemetryStats) {
          setStats((prev) => ({
            ...prev,
            activeHunts: d.telemetryStats.totalHunts || prev.activeHunts,
            openCases: d.telemetryStats.totalCases || prev.openCases,
            totalIOCs: d.telemetryStats.totalIOCs || prev.totalIOCs,
          }));
        }
      })
      .catch(() => {});
    ...
    fetch('/api/coverage')
      .then((r) => r.json())
      .then((d) => {
        if (d && typeof d.coveragePercentage === 'number') {
          setStats((prev) => ({ ...prev, coveragePercent: d.coveragePercentage }));
        }
      })
      .catch(() => {});
    ...
    fetch('/api/soar/actions')
      ...
      .then((d) => {
        if (d && Array.isArray(d.actions)) {
          const pending = d.actions.filter((a: any) => a.status === 'PENDING_APPROVAL').length;
          setStats((prev) => ({ ...prev, pendingSOAR: pending }));
        }
      })
      .catch(() => {});
